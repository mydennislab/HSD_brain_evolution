...
## 5. WGS PnPs analysis (TBD)

```bash
cd /share/dennislab/users/dcsoto/ms_hsd/wgs/pnps
```

```bash
conda activate variants

bedtools groupby -g 4 -c 4 -o count /share/dennislab/users/dcsoto/ms_hsd/00_SDs/genes/CHM13.combined.v4.exons.bed > exons_per_gene.tsv

# selecting SD-98 genes within accessibility mask
cat /share/dennislab/users/dcsoto/ms_hsd/00_SDs/genes/CHM13.combined.v4.genes.SD-98.protein_coding.gene_body.bed /share/dennislab/users/dcsoto/ms_hsd/00_SDs/genes/CHM13.combined.v4.genes.SD-98.unprocessed_pseudogenes.gene_body.bed > SD98_genes.bed

/share/dennislab/users/dcsoto/ms_hsd/00_SDs/genes/CHM13.combined.v4.exons.SD-98.bed

join -t, -j1 -a 1 -a 2 file1 file2
  
# all gene body within accessibility mask
bedtools intersect -f 1 -wa -a SD98_genes.bed -b /share/dennislab/projects/hsd_genes/T2T/regions/combined_mask.bed.gz

# all exons within accessibility mask


# extracting variants in SD-98 genes and accessibility mask
# bcftools view -S ... recalculates INFO/AC and INFO/AN only, but not INFO/AF

cat /share/dennislab/users/dcsoto/ms_hsd/00_SDs/genes/CHM13.combined.v4.genes.SD-98.protein_coding.gene_body.bed /share/dennislab/users/dcsoto/ms_hsd/00_SDs/genes/CHM13.combined.v4.genes.SD-98.unprocessed_pseudogenes.gene_body.bed > SD98_genes.bed

mkdir -p vcf_filt

echo "chr1 chr2 chr3 chr4 chr5 chr6 chr7 chr8 chr9 chr10 chr11 chr12 chr13 chr14 chr15 chr16 chr17 chr18 chr19 chr20 chr21 chr22" | tr ' ' '\n' \
| xargs -n1 -P10 bash -c '
  echo $0
  bedtools intersect -header -a /share/dennislab/projects/hsd_genes/T2T/1kgp/1kgp.$0.recalibrated.snp_indel.pass.vcf.gz -b SD98_genes.bed \
  | bedtools intersect -header -a - -b /share/dennislab/projects/hsd_genes/T2T/regions/combined_mask.bed.gz \
  | bgzip -c \
  > vcf_filt/1kgp.$0.recalibrated.snp_indel.pass.SD-98.mask.vcf.gz
'

# filtering SNPs as follows:
# - select pass only
# - select SNPs only
# - select biallelic sites only
# - select SNPs with no missing data (>50% true genotypes)
# - select SNPs with MAF > 0.01

echo "chr1 chr2 chr3 chr4 chr5 chr6 chr7 chr8 chr9 chr10 chr11 chr12 chr13 chr14 chr15 chr16 chr17 chr18 chr19 chr20 chr21 chr22" | tr ' ' '\n' \
| xargs -n1 -P10 bash -c '
  echo $0
  gunzip -c vcf_filt/1kgp.$0.recalibrated.snp_indel.pass.SD-98.mask.vcf.gz \
  | bcftools norm -m- -f /share/dennislab/projects/t2t/assembly/v1.0/chm13.draft_v1.0.plusY.fasta \
  | bcftools view -f 'PASS,.' \
  | awk "{{if(\$0 ~ \"^#\" || \$5 != \"*\") {{print}} }}" \
  | bcftools view --exclude-types indels \
  | bcftools norm -m+ \
  | bcftools view --max-alleles 2 \
  | vcftools --vcf - --max-missing 0.5 --remove-filtered-all --recode --recode-INFO-all --stdout \
  | bcftools +fill-tags -- -t AF,MAF \
  | bcftools view -i "MAF[0]>0.01" \
  | bgzip -c \
  > vcf_filt/1kgp.$0.recalibrated.snp_indel.pass.SD-98.mask.filt.vcf.gz
'

# annotating synonymous and non-synonymous
conda activate vep
echo "chr1 chr2 chr3 chr4 chr5 chr6 chr7 chr8 chr9 chr10 chr11 chr12 chr13 chr14 chr15 chr16 chr17 chr18 chr19 chr20 chr21 chr22" | tr ' ' '\n' \
| xargs -n1 -P5 bash -c '
vep --force_overwrite \
-i vcf_filt/1kgp.$0.recalibrated.snp_indel.pass.SD-98.mask.filt.vcf.gz  \
-o vcf_filt/1kgp.$0.recalibrated.snp_indel.pass.SD-98.mask.filt.vep.vcf --vcf \
--gff /share/dennislab/projects/hsd_genes/T2T/annotation/CHM13.combined.v4.locus.srt.biotype.gff3.gz \
--fasta /share/dennislab/projects/t2t/assembly/v1.0/chm13.draft_v1.0.plusY.fasta
'

# compressing and indexing
echo "chr1 chr2 chr3 chr4 chr5 chr6 chr7 chr8 chr9 chr10 chr11 chr12 chr13 chr14 chr15 chr16 chr17 chr18 chr19 chr20 chr21 chr22" | tr ' ' '\n' \
| xargs -n1 -P5 bash -c ' 
bgzip -f vcf_filt/1kgp.$0.recalibrated.snp_indel.pass.SD-98.mask.filt.vep.vcf
tabix -f vcf_filt/1kgp.$0.recalibrated.snp_indel.pass.SD-98.mask.filt.vep.vcf.gz
'

# concatenate chromosomes
files=$(for chr in chr1 chr2 chr3 chr4 chr5 chr6 chr7 chr8 chr9 chr10 chr11 chr12 chr13 chr14 chr15 chr16 chr17 chr18 chr19 chr20 chr21 chr22; do echo "vcf_filt/1kgp.${chr}.recalibrated.snp_indel.pass.SD-98.mask.filt.vep.vcf.gz"; done | tr "\n" " ")
vcf-concat ${files} > 1kgp.recalibrated.snp_indel.pass.SD-98.mask.filt.vep.vcf
bgzip -f 1kgp.recalibrated.snp_indel.pass.SD-98.mask.filt.vep.vcf
tabix -f 1kgp.recalibrated.snp_indel.pass.SD-98.mask.filt.vep.vcf.gz 

# separating files by superpopulation
grep "AFR" 1kgp_superpops.txt | cut -f1 > 1kgp_superpops.AFR.txt
grep "EUR" 1kgp_superpops.txt | cut -f1 > 1kgp_superpops.EUR.txt
grep "EAS" 1kgp_superpops.txt | cut -f1 > 1kgp_superpops.EAS.txt
grep "SAS" 1kgp_superpops.txt | cut -f1 > 1kgp_superpops.SAS.txt
grep "AMR" 1kgp_superpops.txt | cut -f1 > 1kgp_superpops.AMR.txt

# FILTER BY minimum AC!
# Pre-filter genes fully contained within accessibility mask
printf "AFR\nEUR\nEAS\nSAS\nAMR" | xargs -n1 -P5 bash -c '
  echo "Processing:" $0
  bcftools view -S 1kgp_superpops.$0.txt 1kgp.recalibrated.snp_indel.pass.SD-98.mask.filt.vep.vcf.gz \
  | bcftools +fill-tags -- -t AF,MAF \
  | bgzip -c \
  > 1kgp.recalibrated.snp_indel.pass.SD-98.mask.filt.vep.$0.vcf.gz
  tabix 1kgp.recalibrated.snp_indel.pass.SD-98.mask.filt.vep.$0.vcf.gz
' 

# iterating over each gene and over each superpopulation, extracting AF and functional impact
for superpop in AFR EUR EAS SAS AMR; do
awk '{print $1":"$2"-"$3"\t"$4}' SD98_genes.bed | xargs -n4 -P1 bash -c '
  superpop='${superpop}'

  bcftools view -r $0 1kgp.recalibrated.snp_indel.pass.SD-98.mask.filt.vep.$superpop.vcf.gz \
  | bcftools query -f "%CHROM\t%POS\t%INFO/AC\t%INFO/AN\t%INFO/AF\t%INFO/MAF\t%INFO/CSQ\n" \
  | awk -v locus=$1 -v superpop=$superpop "{print locus\"\\t\"superpop\"\\t\"\$0}" \
  | grep "synonymous\|missense\|stop" \
' > 1kgp.recalibrated.snp_indel.pass.SD-98.mask.filt.vep.$superpop.tsv
done


# iterating over each gene in total dataset
awk '{print $1":"$2"-"$3"\t"$4}' SD98_genes.bed | xargs -n4 -P1 bash -c '
  cat 1kgp.recalibrated.snp_indel.pass.SD-98.mask.filt.vep.vcf.gz \
  | bcftools query -f "%CHROM\t%POS\t%INFO/AC\t%INFO/AN\t%INFO/AF\t%INFO/CSQ\n" \
  | awk -v locus=$1 "{print locus\"\\tall\\t\"\$0}" \
  | grep "synonymous\|missense\|stop"
' > 1kgp.recalibrated.snp_indel.pass.SD-98.mask.filt.vep.all.tsv
```
